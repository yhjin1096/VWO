...
// #include "VWO/camera/equirectangular.h"
// #include "VWO/camera/radial_division.h"
#include "VWO/data/camera_database.h"
...
void camera_database::from_json(const nlohmann::json& json_cameras) {
    ...
    for (const auto& json_id_camera : json_cameras.items()) {
        ...
        const auto setup_type = Camera::load_setup_type(json_camera.at("setup_type").get<std::string>());
        const auto model_type = Camera::load_model_type(json_camera.at("model_type").get<std::string>());
        const auto color_order = Camera::load_color_order(json_camera.at("color_order").get<std::string>());

        switch (model_type) {
            case Camera::model_type_t::Perspective: {
                camera = new Perspective(camera_name, setup_type, color_order,
...
bool camera_database::from_db(sqlite3* db) {
    ...
    while ((ret = sqlite3_step(stmt)) == SQLITE_ROW) {
        ...
        auto setup_type = Camera::load_setup_type(std::string(p, p + sqlite3_column_bytes(stmt, 2)));
        p = reinterpret_cast<const char*>(sqlite3_column_blob(stmt, 3));
        auto model_type = Camera::load_model_type(std::string(p, p + sqlite3_column_bytes(stmt, 3)));
        p = reinterpret_cast<const char*>(sqlite3_column_blob(stmt, 4));
        auto color_order = Camera::load_color_order(std::string(p, p + sqlite3_column_bytes(stmt, 4)));
