...
void orb_params_database::add_orb_params(orb_params* orb_params) {
...
orb_params* orb_params_database::get_orb_params(const std::string& orb_params_name) const {
    std::lock_guard<std::mutex> lock(mtx_database_);
...
void orb_params_database::from_json(const nlohmann::json& json_orb_params) {
    ...
    for (const auto& json_id_orb_params : json_orb_params.items()) {
        ...
        auto orb_params = new ::orb_params(orb_params_name,
