...
#include "VWO/data/bow_database.h"
#include "VWO/data/camera_database.h"
#include "VWO/data/orb_params_database.h"
...
keyframe::keyframe(const unsigned int id, const double timestamp,
                   const Mat44_t& pose_cw, Camera* camera,
                   const orb_params* orb_params, const frame_observation& frm_obs,
...
std::shared_ptr<keyframe> keyframe::make_keyframe(unsigned int id, const Frame& frm) {
    ...
    ptr->graph_node_ = ::make_unique<graph_node>(ptr);
    return ptr;
}
...
std::shared_ptr<keyframe> keyframe::from_stmt(sqlite3_stmt* stmt,
    ...
    if (camera->setup_type_ != Camera::setup_type_t::Monocular) {
