...
{
    extract(in_image, in_image_mask, keypts, out_descriptors);
}

ORBExtractor::ORBExtractor(const orb_params* orb_params,
...
void ORBExtractor::extract(const cv::_InputArray& in_image, const cv::_InputArray& in_image_mask,
                            std::vector<cv::KeyPoint>& keypts, const cv::_OutputArray& out_descriptors) {
    if (in_image.empty()) {
        return;
    }
...
void ORBExtractor::create_rectangle_mask(const unsigned int cols, const unsigned int rows) {
    if (rect_mask_.empty()) {
...
void ORBExtractor::compute_image_pyramid(const cv::Mat& image) {
...
void ORBExtractor::compute_fast_keypoints(std::vector<std::vector<cv::KeyPoint>>& all_keypts, const cv::Mat& mask) const {
...
std::vector<cv::KeyPoint> ORBExtractor::distribute_keypoints(const std::vector<cv::KeyPoint>& keypts_to_distribute,
...
void ORBExtractor::compute_orientation(const cv::Mat& image, std::vector<cv::KeyPoint>& keypts) const {
    for (auto& keypt : keypts) {
        keypt.angle = ic_angle(image, keypt.pt);
    }
}

void ORBExtractor::correct_keypoint_scale(std::vector<cv::KeyPoint>& keypts_at_level, const unsigned int level) const {
    if (level == 0) {
        return;
    }
...
float ORBExtractor::ic_angle(const cv::Mat& image, const cv::Point2f& point) const {
    return orb_impl_.ic_angle(image, point);
}

void ORBExtractor::compute_orb_descriptor(const cv::KeyPoint& keypt, const cv::Mat& image, uchar* desc) const {
