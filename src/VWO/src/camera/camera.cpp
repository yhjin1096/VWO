...
Camera::setup_type_t Camera::load_setup_type(const YAML::Node& yaml_node) {
    ...
    if (setup_type_str == "monocular") {
        ...
        return setup_type_t::Stereo;
    }
    else if (setup_type_str == "RGBD") {
        return setup_type_t::RGBD;
    }
...
Camera::model_type_t  Camera::load_model_type(const YAML::Node& yaml_node) {
    ...
    if (model_type_str == "perspective") {
        ...
        return model_type_t::Fisheye;
    }
    else if (model_type_str == "equirectangular") {
        return model_type_t::Equirectangular;
    }
    else if (model_type_str == "radial_division") {
        return model_type_t::RadialDivision;
    }
...
bool Camera::is_valid_shape(const cv::Mat& img) const {
    return static_cast<int>(cols_) == img.cols && static_cast<int>(rows_) == img.rows;
}

void Camera::show_common_parameters() const {
...
cv::KeyPoint Camera::undistort_keypoint(const cv::KeyPoint& dist_keypt) const {
...
void Camera::undistort_points(const std::vector<cv::Point2f>& dist_pts, std::vector<cv::Point2f>& undist_pts) const {
...
void Camera::undistort_keypoints(const std::vector<cv::KeyPoint>& dist_keypts, std::vector<cv::KeyPoint>& undist_keypts) const {
...
void Camera::convert_points_to_bearings(const std::vector<cv::Point2f>& undist_pts, eigen_alloc_vector<Vec3_t>& bearings) const {
...
void Camera::convert_keypoints_to_bearings(const std::vector<cv::KeyPoint>& undist_keypts, eigen_alloc_vector<Vec3_t>& bearings) const {
    assert(bearings.size() == 0);
...
void Camera::convert_bearings_to_points(const eigen_alloc_vector<Vec3_t>& bearings, std::vector<cv::Point2f>& undist_pts) const {
