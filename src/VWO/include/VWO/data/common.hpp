 ...
 */
void assign_keypoints_to_grid(const Camera* camera, const std::vector<cv::KeyPoint>& undist_keypts,
 ...
 * @return
 */
auto assign_keypoints_to_grid(const Camera* camera, const std::vector<cv::KeyPoint>& undist_keypts,
                              unsigned int num_grid_cols, unsigned int num_grid_rows)
 ...
 * @return
 */
inline bool get_cell_indices(const Camera* camera, const cv::KeyPoint& keypt,
 ...
 * @return
 */
std::vector<unsigned int> get_keypoints_in_cell(const Camera* camera, const std::vector<cv::KeyPoint>& undist_keypts,
...
std::vector<unsigned int> get_keypoints_in_cell(const Camera* camera, const frame_observation& frm_obs,
                                                const float ref_x, const float ref_y, const float margin,
                                                const int min_level = -1, const int max_level = -1);
 ...
 */
Vec3_t triangulate_stereo(const Camera* camera,
